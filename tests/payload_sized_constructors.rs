use bplustree::{
    BPlusTreeMap, RECOMMENDED_BRANCH_PAYLOAD_BYTES_INTERNAL,
    RECOMMENDED_LEAF_PAYLOAD_BYTES_INTERNAL,
};
...
fn recommended_selects_capacities_from_u64_slot_sizes() {
    ...
    let expected_branch_cap =
        RECOMMENDED_BRANCH_PAYLOAD_BYTES_INTERNAL / (size_of::<u64>() + size_of::<*mut u8>());
...
fn recommended_scales_down_for_wider_types() {
    type Wide = [u8; 64];
    let mut tree = BPlusTreeMap::<Wide, Wide>::new();

    assert_eq!(tree.leaf_layout().cap, 4);
    let expected_branch_cap =
        RECOMMENDED_BRANCH_PAYLOAD_BYTES_INTERNAL / (size_of::<Wide>() + size_of::<*mut u8>());
    assert_eq!(tree.branch_layout().cap as usize, expected_branch_cap);
...
fn custom_targets_scale_each_node_kind_independently() {
    let tree = BPlusTreeMap::<u64, u64>::with_payload_targets(1_024, 2_048);
...
fn targets_are_clamped_to_supported_capacities() {
    let tiny = BPlusTreeMap::<u64, u64>::with_payload_targets(1, 1);
    assert_eq!(tiny.leaf_layout().cap, 4);
    assert_eq!(tiny.branch_layout().cap, 4);

    let huge = BPlusTreeMap::<u64, u64>::with_payload_targets(usize::MAX, usize::MAX);
...
fn recommended_tree_supports_normal_map_operations() {
    ...
    assert_eq!(RECOMMENDED_LEAF_PAYLOAD_BYTES_INTERNAL, 512);
    assert_eq!(RECOMMENDED_BRANCH_PAYLOAD_BYTES_INTERNAL, 4_096);
